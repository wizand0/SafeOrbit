    ...
    "servers": {
      "$serverId": {
        ...
        "ownerUid": {
          ...
          ".write": "(data.val() == null)"  // только при создании
        },
        
        "pairing": {
          ".read": "auth != null && root.child('servers').child($serverId).child('ownerUid').val() === auth.uid",  // только владелец может читать pairing данные
          
          ".write": "auth != null && root.child('servers').child($serverId).child('ownerUid').val() === auth.uid && (data.child('tokenHash').val() == null)",  // нельзя изменить токен после создания
          ...
          "consumed": {
            ".read": "auth != null && root.child('servers').child($serverId).child('ownerUid').val() === auth.uid",
            ".write": "auth != null && root.child('servers').child($serverId).child('ownerUid').val() === auth.uid && (data.val() == false || data.val() == null) && newData.val() == true"  // только однократное изменение на true
          }
        },
